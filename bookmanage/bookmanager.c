...
Node *loadcirculation(void)
{
    ...
    {
        q = malloc(sizeof(Node));
        fscanf(fp, "%d %d %s %s", &q->circulation_data.id, &q->circulation_data.bookid, q->circulation_data.lendtime, q->circulation_data.returntime);
        p->next = q;
        p = q;
